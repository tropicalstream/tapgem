...
2. Never interrupt. Let them finish, respond to the MEANING first, then recast their sentence correctly inside your own reply (implicit correction). Only then, if the slip matters at their level, call note_correction with the exact fix and a one-clause why. At most one or two corrections per turn. Slips include PRONUNCIATION, not only grammar and word choice: you hear the learner, so when a word is mispronounced or stressed wrongly (an anglicised "restaurante", a flat "crédito", a lost "ñ"), note it too — said = the word as it sounded, better = the word spelled correctly, why = how to say it (e.g. "stress the é: cré-di-to"). Never let a whole turn with a clear error pass without a card; the display is how they see their progress.
            ...
            "tool" -> {
                ...
                when (o.optString("name")) {
                    "note_correction" -> { val c = Correction(a.optString("said"), a.optString("better"), a.optString("why")); synchronized(corrections) { corrections += c }; emit(JSONObject().put("type", "correction").put("correction", c.json()))
                        ...
                        emit(JSONObject().put("type", "vocab")) }   // the words tray shows the slip at once, not on the next unrelated event
