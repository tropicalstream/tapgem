...
object Bookmarks {
    ...
    /** [origin] = the live widget's source when saved — how "already bookmarked" is recognised. */
    data class Bookmark(val id: String, val title: String, val widget: Widget, val createdAt: Long, val thumb: File?, val origin: String = widget.source) {
    ...
    fun save(w: Widget, thumb: Bitmap?, name: String? = null): Bookmark {
        ...
        val existing = list().firstOrNull { it.origin == w.source && it.title.equals(title, ignoreCase = true) }
    ...
    fun materialize(b: Bookmark): Widget {
        ...
        val src = if (w.source.startsWith(filesDir.absolutePath)) {
            ...
            val out = File(target, f.name.substringAfter('-').ifBlank { f.name }.let { n -> "bm_${b.id}_$n" })
    ...
    private fun keepFile(source: String, id: String): String {
        ...
        val out = File(filesDir, "$id-${f.name}")
        return runCatching { f.copyTo(out, overwrite = true); out.absolutePath }.getOrElse { Log.w(TAG, "keep: ${it.message}"); source }
    }

    private fun toJson(b: Bookmark) = JSONObject().put("id", b.id).put("title", b.title).put("createdAt", b.createdAt).put("origin", b.origin).put("widget", b.widget.toJson())

    private fun fromJson(o: JSONObject): Bookmark? {
        val w = Widget.fromJson(o.getJSONObject("widget")) ?: return null
        val id = o.optString("id").ifBlank { return null }
        return Bookmark(id, o.optString("title").ifBlank { w.title }, w, o.optLong("createdAt"), File(dir, "$id.png").takeIf { it.exists() }, o.optString("origin").ifBlank { w.source })
    }
}
