...
class ToolDispatcher(context: Context) {
    ...
    private val tools: Map<String, AiTool> = listOf(
        ...
        AppBuilderTool(context), MediaTool(context), WebTool(context), BookmarkTool(), IrcTool(context), DiscordTool(context), InterpreterTool(context), MusicTool(context), TutorTool(context)
    ).associateBy { it.name }
