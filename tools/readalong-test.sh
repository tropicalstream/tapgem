...
args=json.dumps({"action":"eval","target":"Read-along","js":js})
print("am broadcast -a com.tapgem.app.TOOL --es name web --es args " + "'" + args.replace("'", "'\\''") + "'")
PY
$A shell < "$OUT/.ev.sh" >/dev/null 2>&1; sleep 2
$A logcat -d -t 600 'TapGemApp:*' '*:S' 2>/dev/null | grep "TOOL web → $N|" | tail -1 | sed "s/.*TOOL web → $N|//"; }
...
$A logcat -d 'TapGemApp:*' '*:S' 2>/dev/null | grep "TOOL web" | cut -c1-140 | tee -a "$OUT/run.log"
...
echo "== stopping, pulling record ==" | tee -a "$OUT/run.log"
...
$A logcat -d -s BookReader GeminiRest WebTool WebCommandBus 2>/dev/null > "$OUT/reader.log"
